...
2. **Recon report template**: Read `{SCHEMA_PATH}` (templates/recon-report.md) for the exact LOD-2 output format for your step
3. **Your assigned step**: Check TaskList, claim a `Group 0:` task, read its description for your specific instructions
...
1. Call TaskList and claim an available `Group 0:` task (set owner to your name)
...
7. Check TaskList for more available Group 0 tasks — if any are unblocked, claim and execute them
