...
3. **Detect plugins**: Scan `plugins/` directory and match framework-specific plugins against the recon step files (same as full analysis — plugins are injected into whichever agents are selected)
...
5. **Run selected agents** in wave order (respect dependencies between phases)
